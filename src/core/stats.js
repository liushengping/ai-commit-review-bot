/**
 * Review statistics tracker - platform-agnostic
 */
const fs = require('fs');
...
function loadStats(repoRoot) {
  ...
  return {
    version: 1, totalReviews: 0, totalIssues: 0,
...
function recordReview(repoRoot, review, meta = {}) {
  ...
  for (const issue of review.issues) {
    if (issue.severity in stats.issuesBySeverity) stats.issuesBySeverity[issue.severity]++;
    if (issue.category in stats.issuesByCategory) stats.issuesByCategory[issue.category]++;
  }

  if (review.risk_level in stats.riskDistribution) stats.riskDistribution[review.risk_level]++;

  stats.reviews.push({
    timestamp: new Date().toISOString(),
    pr: meta.prNumber || null, pr_title: meta.prTitle || null,
    risk_level: review.risk_level, issue_count: review.issues.length,
    summary: review.summary, files_reviewed: meta.filesReviewed || 0,
    files_skipped: meta.filesSkipped || 0, model: meta.model || null,
    duration_seconds: meta.duration || null,
  });

  if (stats.reviews.length > 100) stats.reviews = stats.reviews.slice(-100);

  const statsPath = path.join(repoRoot, STATS_FILE);
  try { fs.writeFileSync(statsPath, JSON.stringify(stats, null, 2), 'utf8'); } catch (e) {}
